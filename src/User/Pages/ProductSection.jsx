    ...
    <section className="ps-discovery-section">
      ...
      <div className="ps-filter-nav">
        <button 
          ...
          className={`ps-filter-btn ${activeTab === 'latest' ? 'ps-active-filter' : ''}`}
        >
          Latest Products
        </button>
        <button 
          onClick={() => setActiveTab('discount')} 
          className={`ps-filter-btn ${activeTab === 'discount' ? 'ps-active-filter' : ''}`}
        >
          Stores with Discount
        </button>
        <button 
          onClick={() => setActiveTab('top')} 
          className={`ps-filter-btn ${activeTab === 'top' ? 'ps-active-filter' : ''}`}
        >
          Top Selling Stores
        </button>
      </div>
                ...
                <div key={product.id} className="ps-product-card">
                  <div className="ps-product-media">
                    ...
                    <div className="ps-action-buttons">
                      <button className="ps-action-btn ps-quick-view">
                        <i className="far fa-eye"></i> Quick View
                      </button>
                      <button className="ps-action-btn ps-wishlist">
                  ...
                  <div className="ps-product-details">
                    ...
                    <button className="ps-add-to-cart">
          ...
          {activeTab === 'top' && (
            <div className="ps-product-grid">
              {topStores.map((store) => (
                <div key={store.id} className="ps-product-card">
                  <div className="ps-product-media">
                    <img  src={`http://127.0.0.1:8000/storage/logo/${store.logo_url}`} alt={store.store_name} className="ps-product-image" />
                    <div className="ps-badge-group">
                  ...
                  <div className="ps-product-details">
                    <h3 className="ps-product-title">{store.store_name}</h3>
                    <div className="ps-price-tag">
                      {store.completed_orders_count} Orders
                    </div>
                    <button className="ps-add-to-cart">
                      <i className="fas fa-store"></i> Visit Store
                    </button>
