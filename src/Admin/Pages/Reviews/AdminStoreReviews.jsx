...
const AdminStoreReviews = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const res = await axios.get('http://127.0.0.1:8000/api/admin/reviews/store', {
        headers: {
  ...
  const deleteReview = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://127.0.0.1:8000/api/admin/reviews/store/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        const currentCount = reviews.slice(firstIndex, lastIndex).length;
...
export default AdminStoreReviews;
